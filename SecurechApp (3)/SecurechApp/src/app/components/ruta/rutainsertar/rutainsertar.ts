...
export class Rutainsertar implements OnInit {
  ...
  aceptar(): void {
    if (this.form.valid && this.rutaTrazada) {
      ...
      this.ruta.usuario = this.listaUsuarios.find(u => u.id_usuario === idUsuario) || new Usuario();
      this.ruta.tipoVehiculo = this.listaTipovehiculos.find(t => t.id_tipovehiculo === idTipo) || new tipovehiculo();
  ...
  // ==============================
  onOrigenSeleccionado(data: any) {
    this.form.patchValue({
      origen: data.nombre,
      origen_latitud: data.latitud,
      origen_longitud: data.longitud,
    });
  }

  onDestinoSeleccionado(data: any) {
    this.form.patchValue({
      destino: data.nombre,
      destino_latitud: data.latitud,
      destino_longitud: data.longitud,
    });
  }

  onRutaTrazada(valor: boolean) {
    this.rutaTrazada = valor;
  }
